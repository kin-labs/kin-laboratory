export const TEST_SECRET_KEY =
  '4j3PbCCYvcAz1FbKGs7fBoQ5cd8piWCsQm5k6wNnTTzEtE6aM8JZ2AJaaJTjZJgGk9LywyonNHcVopHAwrMqh6kr';
export const TEST_PUBLIC_KEY = '5ZWj7a1f8tWkjBESHKgrLmXshuXxqeY9SYcfbshpAqPG';
export const TEST_STELLAR_SECRET_KEY =
  'SBB3CCFLL5WKQE2ZPRHSULEFL2623BNDYPHTJVEEXJCTOYNBOZHZUFEF';
export const TEST_STELLAR_PUBLIC_KEY =
  'GD3UHL53AXEGRKUGE2H2TOKVUQM2UEZG4ULM7H4YWFNPQTG35EW5SCPA';
export const TEST_STELLAR_PUBLIC_KEY_TO_SOLANA =
  'HeDZgu4ee1PJiU7EB2oBFFphBAcUDk5MXgaDBzmDrTEQ';
export const TEST_STELLAR_SECRET_KEY_TO_SOLANA =
  '2MVjrkqAp7KeSSkXn3ivmJTKhw2BF9TFCbqxSC91XveNnubZbanKBvANRBcyvFPpo429g9cQFoReb8s1Rgsuo7t4';

export const TEST_MNEMONIC_12 =
  'pill tomorrow foster begin walnut borrow virtual kick shift mutual shoe scatter';
export const TEST_MNEMONIC_24 =
  'grab amused tattoo cruise industry corn welcome wealth tilt erupt gauge ankle remove toast journey heavy unit vibrant zoo blood notice jealous gesture cargo';
export const TEST_MNEMONIC_24_SECRET_KEY =
  'RkXUT99dpuSPP33w3X6tTx8KTDJMs1ce3tjeHWm2GSTKau7qFDN5BFMEeGToMGcEDunXuBpozxWAnr6uir8JVMD';
export const TEST_MNEMONIC_24_PUBLIC_KEY =
  '36hssW7DqagnHyC3dPjD59kjiUnYWrYd8hXeQ3zZ5kUK';
export const TEST_MNEMONIC_24_SET = [
  {
    publicKey: '6pFBagvvyvBHuCkbMAiPTLn42nnHXrHC2zwWSdZ1NtVn',
    secretKey:
      'WTqijcBccatWsAA6WJzaqgNzJVdARiBHceL5DQG15RbZCNn9jeoEjwMyKRiQqsPvLKGhgkQMoUgo2ybbZmStU3r',
  },
  {
    publicKey: 'Foo948ttNuYa8SsfHX78BVyPVA7P7MsV8u43ZeQ1RBxm',
    secretKey:
      'TcauP27RkmyhH2AsVaPh5rCofCN4xCoB1h5MgUT6eRoJ7GiVPDBqtW1dAmh9CNcHSqCVnoEFUbRhfApM6oLrUDF',
  },
  {
    publicKey: '3ySWEh9mvVUpzMYYcqEv7VeAQp7ueGFAjUg1G9DBTjD5',
    secretKey:
      '48wHgpEEb1f14kcYAyskTmUY9WnzLXuGUi1qmGiZr89uEmNo8g3uDNsk4aNjMaAfrE4oew7tk7VXWtUj8jWZGdbR',
  },
  {
    publicKey: 'HyMtaWzweBgpmgDf2dn8RhsM1c9m48VPQFjV6DAkHxbv',
    secretKey:
      '46HfvZCsmSCnS9akRqULDci7313KQGk7Ndv7PXejTa4rnEtH7PnGMUo5jxkJwMJjifFxHyfJSkYcziEBqJqGPQYL',
  },
  {
    publicKey: '5SZcAkce1YHt5D4ANfxQGBo1fRcjn7jbCaUGgaBPz8or',
    secretKey:
      '3Gr9wYRLhz9LLhkSTYy1TsXyUbDcsfhVUxpDxDrHSjdY2J1tNBQWRZ4qdFvbH5AiMJCvq9qhxZc6j1RinPXMWDMa',
  },
  {
    publicKey: '77VRnnzwRTeQLDpNDfMh8DrDkzbAtcuKncZyWN49oegG',
    secretKey:
      'vrMzreRiV8Abz2WxqZhgh2Hmg5Mnc65QD3P1tr5Y7a296d7Y4FrUBppGCUAz2qCa6iNVUjygZu4SMiRDmW59Vbe',
  },
  {
    publicKey: 'GRwsvJC92A9UB4eEcpzuxPqsuBdKRjd731uGp6BLbPJv',
    secretKey:
      '4ZPFuK3dS189sJFXdTiVRXeVA7ZV4LPgUUgXJwx7WVWMVCpafG3L1D2j1Z4dSJzamyYruHCSYWrwP6s8HhGLTYS2',
  },
  {
    publicKey: 'piCoBHqPoK4dhg84g1NajB6vdJX785G3wT2rExaQ6Q3',
    secretKey:
      '3iUkSFmpVM4rZUsMJqGH1ajY3tWxG9A64agQwaPCZ1m5Yc3V8JacEPnAu3oDdhxTARHnpXKBBVcDJhv9cR2JDvd9',
  },
  {
    publicKey: '6EeiyS9X6222PeUzpKA32jc3ZEkpTKms3qNZEmySm1DS',
    secretKey:
      '8BJqbw31Zmn7887LtFtevmmGaUm5BNUvMB1tsnLdzCv5TFPTodYDCsKCFovHoL1BSZwGxbixDZoWwszLpSyYKXv',
  },
  {
    publicKey: 'DhW8bCabXWMJYg99rtZyy2XWax8J5wfLh8kmkpDxbR8G',
    secretKey:
      '3P4zpjXuBtxH8hMguo8k68Nz3mDYRto3CaMkSm55TtPZzinRYg76uyvy2CBimcegBRThH92H9MzffYShJB62W8ya',
  },
];

export const TEST_MNEMONIC_12_SET = [
  {
    publicKey: '5F86TNSTre3CYwZd1wELsGQGhqG2HkN3d8zxhbyBSnzm',
    secretKey:
      'cWNhG6WhR4q6X5v8d65x6UgVR4buQJFkpKVvKiFDbbbZnoxpTJZLoCkCLZXpCYKc1QgyXYbhQpACYN8VKgS5xxq',
  },
  {
    publicKey: 'AWjbG5SH5VEay5ksZbGHHgJhYRhM1rsN5Z538cfFvs4a',
    secretKey:
      'CnJAJFfB8PkLPAGVfc7Pc9J2cWaCAiN65j44DFDP8ub7R1jN1XRkgT7FFBdBy9pUB33UmdAskuB5ZLPspH6U9jL',
  },
  {
    publicKey: 'TdFkuCzMB6ikLWbhxXkHT8ZZFdqD7LrD5QbuRYQCp18',
    secretKey:
      '5ZKRCfEZpp5gchtkJTFJpPJQcbjR4M99L2mYRRcAWdXCE4Gfvd7CwdZgvDHcMeMTyGBLKiVEoqQ7etnGy3JnXtwc',
  },
  {
    publicKey: 'C7xHnBLteo5QZwc1Dzgu2aMvPVd5FwsX44cH6DMMszPz',
    secretKey:
      '4mKLiQGyBvp9BGoF1FUtvLTLc2v9UormC6Aqva7eM2nTFnMUWj5KaTCSELUMyp4otJ253oZnJHApjUFV6CB7rQ7i',
  },
  {
    publicKey: '7QJUf8m94wnSrukK7HQsbBZ2V4NVQQUuvZ5wdux9a5Lu',
    secretKey:
      '4YudKZdE1MHpFJuXjKZGDERJ8Zh4e3wY3RmPw7csm3dmDTsd1iKtoyFuSYA6kmEzDTmLT8Dz68NUQNoe3jYQZexZ',
  },
  {
    publicKey: 'GkeNvLEAznNwPV47rXahTuYtqqCpzFs1g3gb2jq7x8Kd',
    secretKey:
      '4MqYS4wyuxJsR48yvCy4jULnqAFxLscsUVW3eMoHzBPSNKyAX8rXtkEg3TQkgznN1G6iNfJHTf3mPhrvFh7bhyKR',
  },
  {
    publicKey: '8T1j8WyQc726rByayWTz1tZZwMV2A6U1C4Ase2FxHgib',
    secretKey:
      'Bb8z3xU7NmUxoX716NoZBUWXK8bciXJdZnknbgsUjaFm8S1zaFGipzYQTT2bf8x3xk79VjhP6KrfevwTEMZSZ6F',
  },
  {
    publicKey: 'CV2sNJvxqdHRQW2pejnGjLMqWXBy61jnx3uExxZ7n3CW',
    secretKey:
      '5sJkm11xJgsztEHgPGNMRwVzBHwhJtf6kNaH9MBCwW6JiTe5aBDDy142JuM48rmbhuVVRmfgx9QDRYKbRRN9xvyp',
  },
  {
    publicKey: '32e2ughnjjJ9MxuBb7JystX81Mh52qNfm8Kh9nMPXYtM',
    secretKey:
      '5PwFc5bJ2cxMe8zeQRZzyL1Md3KAxh4pBVbv4Myo9m6HaPEnYvT7SDmYY8tuvXuHy1JZYN86L5pNnG9J6LfGxcLD',
  },
  {
    publicKey: '36herGLdWaFZ9raFLeXBLE43t1oMwgd4yoUAAKaSTHKR',
    secretKey:
      'ByHWNEnSCyoz3LzqnV9xRoKMKEWW9thGXEUp5XCP863ymhk7zhd54j8svVxgrKPDa9yboLbdobQMEuX7s4X4SsV',
  },
];

export const TEST_STELLAR_BYTEARRAY = [
  67, 177, 8, 171, 95, 108, 168, 19, 89, 124, 79, 42, 44, 133, 94, 189, 173,
  133, 163, 195, 207, 52, 212, 132, 186, 69, 55, 97, 161, 118, 79, 154, 247, 67,
  175, 187, 5, 200, 104, 170, 134, 38, 143, 169, 185, 85, 164, 25, 170, 19, 38,
  229, 22, 207, 159, 152, 177, 90, 248, 76, 219, 233, 45, 217,
];

export const TEST_SECRET_BYTEARRAY = [
  186, 78, 68, 54, 63, 205, 1, 141, 2, 89, 45, 80, 77, 168, 215, 120, 56, 57,
  72, 222, 50, 140, 31, 236, 254, 35, 208, 163, 138, 186, 225, 18, 67, 194, 241,
  235, 28, 5, 209, 235, 248, 58, 150, 42, 218, 71, 43, 177, 183, 62, 55, 96,
  216, 41, 59, 146, 121, 132, 223, 24, 39, 109, 3, 163,
];
